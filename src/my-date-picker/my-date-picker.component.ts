...
import {IMyDate, IMyWeek, IMyMonth, IMyDay, IMyDayLabels, IMyMonthLabels} from './interfaces/index';
...
export class MyDatePicker implements OnChanges {
        ...
        for (let i = 1; i < 7; i++) {
            ...
            if (i === 1) {
                ...
                for (var j = pm; j <= dInPrevM; j++) {
                    ...
                    let currDay = this.isCurrDay(j, m, y, cmo);
                ...
                for (var j = 0; j < daysLeft; j++) {
                    ...
                    let currDay = this.isCurrDay(dayNbr, m, y, cmo);
                    days.push({dateObj: date, cmo: cmo, currDay: currDay, dayNbr: this.getDayNumber(date), disabled: this.isDisabledDay(date)});
                    dayNbr++;
                }
            }
    ...
    getWeekNumber(myDate: IMyDate): IMyWeek {
        let onejan: any = new Date(myDate.year,0,1),
            tDate: any = new Date(myDate.year, myDate.month - 1, myDate.day),
            dow: number = tDate.getDay();
        return {
            days: <IMyDay[]>[],
