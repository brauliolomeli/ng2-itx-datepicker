...
var MyDatePicker = (function () {
    ...
    MyDatePicker.prototype.ngOnChanges = function (changes) {
        if (changes.hasOwnProperty('selDate')) {
            ...
            this.selectedWeek = this.getWeekNumber(this.selectedDate);
        }
        ...
        for (var i = 1; i < 7; i++) {
            ...
            if (i === 1) {
                ...
                for (var j = pm; j <= dInPrevM; j++) {
                    ...
                    var currDay = this.isCurrDay(j, m, y, cmo);
                    days.push({ dateObj: date, cmo: cmo, currDay: currDay, dayNbr: this.getDayNumber(date), disabled: this.isDisabledDay(date) });
                }
                cmo = this.CURR_MONTH;
                var daysLeft = 7 - days.length;
                for (var j = 0; j < daysLeft; j++) {
                    var date = { year: y, month: m, day: dayNbr };
                    var currDay = this.isCurrDay(dayNbr, m, y, cmo);
                    days.push({ dateObj: date, cmo: cmo, currDay: currDay, dayNbr: this.getDayNumber(date), disabled: this.isDisabledDay(date) });
                    dayNbr++;
                }
            }
            ...
            var week = {
                days: days,
                weekNumber: this.getWeekNumber(days[5].dateObj).weekNumber
            };
            if (days[0].cmo < 3) {
    ...
    MyDatePicker.prototype.getWeekNumber = function (myDate) {
        var onejan = new Date(myDate.year, 0, 1), tDate = new Date(myDate.year, myDate.month - 1, myDate.day), dow = tDate.getDay();
        return {
            days: [],
            weekNumber: (myDate.year * 100) + Math.ceil((((tDate - onejan) / 86400000) + onejan.getDay() + 1) / 7),
            start: new Date(myDate.year, myDate.month - 1, myDate.day - dow),
            end: new Date(myDate.year, myDate.month - 1, myDate.day - dow + 6)
        };
    };
    __decorate([
        core_1.Input(), 
    ...
    ], MyDatePicker.prototype, "events", void 0);
    __decorate([
        core_1.Output(), 
    ...
    MyDatePicker = __decorate([
        core_1.Component({
            ...
            styles: [".mydp,.mydp .selector{background-color:#1b75bb}.mydp .nextmonth,.mydp .prevmonth,.mydp .sundayDim{opacity:.5}.mydp{min-width:100px;line-height:1.1;display:inline-block;position:relative;height:auto}.mydp *{padding:0;margin:0}.mydp .selector{margin-top:2px;margin-left:-1px;position:absolute;width:375px;padding:3px;z-index:100}.mydp .alignselectorright{right:-1px}.mydp .selectiongroup{position:relative;display:table;border:none}.mydp .selection{display:table-cell;position:absolute;width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center}.mydp .invaliddate,.mydp .invalidmonth,.mydp .invalidyear{background-color:#F1DEDE}.mydp ::-ms-clear{display:none}.mydp .selbtngroup{position:relative;vertical-align:middle;white-space:nowrap;width:1%;display:table-cell;text-align:right;font-size:0}.mydp .btnclear,.mydp .btnpicker{height:100%;width:30px;border:none;padding:0;cursor:pointer;outline:0;font:inherit;-moz-user-select:none}.mydp .header{width:100%;height:86px;margin-bottom:1px}.mydp .caltable td,.mydp .caltable th{cursor:pointer;height:44px}.mydp .header td{vertical-align:middle;border:none}.mydp .header td:nth-child(1){font-size:16px;padding-left:4px}.mydp .header td:nth-child(2){text-align:center}.mydp .header td:nth-child(3){font-size:16px;padding-right:4px}.mydp .caltable{table-layout:fixed;width:100%;font-size:14px}.mydp .caltable,.mydp .caltable td,.mydp .caltable th{border-collapse:collapse;color:#fff;line-height:1.1}.mydp .caltable td,.mydp .caltable th{padding:5px;text-align:center}.mydp .caltable th{font-size:13px;vertical-align:middle;font-weight:400}.mydp .caltable td div{height:41px;position:static}.mydp .caltable td div span.caption{position:relative;top:13px}.mydp .caltable td div div.has-events{background-color:#fff;position:relative;top:15px;left:19px;height:5px;width:5px}.mydp .inlinedp{position:relative}.mydp .disabled{cursor:default!important;color:#CCC!important}.mydp .currday div{border:1px solid #fff;border-radius:30px}.mydp .selectedday div{border:1px solid #1b75bb;border-radius:30px}.mydp .selectmenu{height:24px;width:60px}.mydp .headerbtn{cursor:pointer;width:29px}.mydp .headertodaybtn{padding:0 4px;cursor:pointer;font-size:11px;min-width:60px;height:22px}.mydp button::-moz-focus-inner{border:0}.mydp .btnclear:focus,.mydp .btnpicker:focus{background:#ADD8E6}.mydp .icon-calendar,.mydp .icon-cross{font-size:16px}.mydp table{display:table}.mydp table td{padding:0}.mydp .monthlabel,.mydp .yearlabel{cursor:pointer}.mydp .monthinput,.mydp .yearinput{width:40px;height:22px;text-align:center;outline:0}.mydp .headerbtn span:hover{width:31px}.mydp .monthlabel:hover,.mydp .yearlabel:hover{color:#8BDAF4;font-weight:700}@font-face{font-family:mydatepicker;src:url(data:application/x-font-ttf;charset=utf-8;base64,) format('truetype');font-weight:400;font-style:normal}.mydp .icon{font-family:mydatepicker;font-style:normal;font-weight:400;font-variant:normal;text-transform:none;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}.mydp .icon-calendar:before{content:\"\\e600\"}.mydp .icon-cross:before{content:\"\\e602\"}.mydp .icon-left:before{content:\"\\e605\"}.mydp .icon-right:before{content:\"\\e607\"}.header-cointainer{display:flex;justify-content:space-around;font-size:24px;color:#fff;font-weight:400}span.next:after,span.next:before,span.previous:after,span.previous:before{border:solid transparent;content:\" \";height:0;width:0;position:absolute;pointer-events:none}span.next:after{border-left-color:#1b75bb;border-width:8px;margin-top:5px}span.next:before{border-color:rgba(194,225,245,0);border-left-color:#fff;border-width:9px;margin-top:4px}span.previous:after{border-right-color:#1b75bb;border-width:11px;margin-top:2px}span.previous:before{border-color:rgba(194,225,245,0);border-right-color:#fff;border-width:9px;margin-top:4px}tr.selectedWeek td{background-color:#5D9CCB}tr.selectedWeek td:nth-child(1){border-top-left-radius:29px;border-bottom-left-radius:29px}tr.selectedWeek td:last-child{border-top-right-radius:29px;border-bottom-right-radius:29px}tr td div{border-radius:20px}"],
            template: "<div class=\"mydp\" [ngStyle]=\"{'width': width, 'border': inline ? 'none' : '1px solid #AAA'}\"><div class=\"selectiongroup\" *ngIf=\"!inline\"><input type=\"text\" class=\"selection\" maxlength=\"10\" [ngClass]=\"{'invaliddate': invalidDate&&indicateInvalidDate}\" placeholder=\"{{showDateFormatPlaceholder?dateFormat:''}}\" [ngStyle]=\"{'height': height, 'line-height': height, 'font-size': selectionTxtFontSize, 'border': 'none', 'padding-right': selectionDayTxt.length>0 ? '60px' : '30px'}\" (keyup)=\"userDateInput($event)\" [value]=\"selectionDayTxt\"> <span class=\"selbtngroup\" [style.height]=\"height\"><button type=\"button\" class=\"btnclear\" *ngIf=\"selectionDayTxt.length>0\" (click)=\"removeBtnClicked()\"><span class=\"icon icon-cross\" [ngStyle]=\"{'line-height': height}\"></span></button> <button type=\"button\" class=\"btnpicker\" (click)=\"openBtnClicked()\"><span class=\"icon icon-calendar\" [ngStyle]=\"{'line-height': height}\"></span></button></span></div><div class=\"selector\" *ngIf=\"showSelector||inline\" [ngClass]=\"{'inlinedp': inline, 'alignselectorright': alignSelectorRight}\"><table class=\"header\"><tr><td><div class=\"header-cointainer\"><div class=\"headerbtn\" (click)=\"prevMonth()\"><span class=\"previous\"></span></div><div class=\"headermonthtxt\"><input type=\"text\" *ngIf=\"editMonth\" class=\"monthinput\" maxlength=\"4\" [inputFocus] [value]=\"visibleMonth.monthTxt\" (keyup)=\"userMonthInput($event)\" (click)=\"$event.stopPropagation()\" [ngClass]=\"{'invalidmonth': invalidMonth}\"> <span [ngClass]=\"{'monthlabel': editableMonthAndYear}\" *ngIf=\"!editMonth\" (click)=\"editMonthClicked($event)\">{{visibleMonth.monthTxt}} {{visibleMonth.year}}</span></div><div class=\"headerbtn\" (click)=\"nextMonth()\"><span class=\"next\"></span></div></div></td></tr></table><table class=\"caltable\"><thead><tr><th *ngFor=\"let d of weekDays\">{{d}}</th></tr></thead><tbody><tr *ngFor=\"let w of dates\" [class.selectedWeek]=\"selectedWeek.weekNumber === w.weekNumber\"><td *ngFor=\"let d of w.days\" [ngClass]=\"{'currmonth':d.cmo===CURR_MONTH&&!d.disabled, 'currday':d.currDay, 'selectedday':selectedDate.day===d.dateObj.day && selectedDate.month===d.dateObj.month && selectedDate.year===d.dateObj.year && d.cmo===CURR_MONTH, 'disabled': d.disabled, 'tablesingleday': d.cmo===CURR_MONTH&&!d.disabled}\" (click)=\"cellClicked(d);$event.stopPropagation()\"><div [ngClass]=\"{'prevmonth':d.cmo===PREV_MONTH,'currmonth':d.cmo===CURR_MONTH,'nextmonth':d.cmo===NEXT_MONTH,'sunday':d.dayNbr === 0 && sunHighlight}\"><span class=\"caption\" [ngClass]=\"{'sundayDim': sunHighlight && d.dayNbr === 0 && (d.cmo===PREV_MONTH || d.cmo===NEXT_MONTH || d.disabled)}\">{{d.dateObj.day}}</span><div class=\"has-events\" *ngIf=\"events.indexOf(formatDate(d.dateObj)) >= 0\"></div></div></td></tr></tbody></table></div></div>",
